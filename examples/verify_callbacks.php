...
$tempDir = __DIR__ . '/../temp/verify_' . bin2hex(random_bytes(4));
...
$mtman->addTask(function () use ($tempDir) {
    ...
    $attempt++;
    file_put_contents($file, $attempt, LOCK_EX);
    if ($attempt < 2) {
...
echo 'Results: ' . json_encode($output['results']) . "\n";
echo 'Errors: ' . json_encode($output['errors']) . "\n";
